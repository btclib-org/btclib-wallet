...
#
# btclib's own profile, five times the hypothesis default of examples on a
# run that happens at every commit. It is not the number that finds a
# latent defect, though: a defect a search turns up belongs in a vector
# test rather than in a search that may or may not repeat it. Deep
# exploration is what the profile below is for.
...
def generated_files_dir(request: pytest.FixtureRequest) -> Path:
    ...
    restates the directory layout once per module, in terms of
    `__file__`, so it moves with the test rather than with the data.
    `request.path` is the asking module's file.
    """
    return Path(request.path).parent / "_generated_files"


# What the json_golden fixture hands a test: named here so that the
...
def json_golden(
    ...
    what `to_dict` writes changes: they are the output of dataclasses
    ...
    What these tests assert about btclib_wallet is `from_dict(to_dict()) ==
    obj`, in memory, beside every golden check.
    """
...
def _skip_what_needs_the_bindings(items: list[pytest.Item]) -> None:
    ...
    than about which build ran it: a run with the bindings installed,
    which is what the floor is measured on, reaches nothing here through
    the hook.
    """
...
def pytest_collection_modifyitems(items: list[pytest.Item]) -> None:
    ...
    both ways out are taken whatever the machine was built with.
    """
    if not INSTALLED:
