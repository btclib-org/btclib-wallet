...
"""Tests for the `btclib_wallet.bip322` module.
...
# none of the four reads a WIF (issue btclib-org/btclib#1188)
...
def _address(wif: str, script_type: str) -> str:
    ...
    (issue btclib-org/btclib#1188), and the parsed key works for all four alike.
    """
...
def test_proof_of_funds_vector(case: dict[str, Any]) -> None:
    ...
    issue btclib-org/btclib#513. So these two are also the test that the refusal
    is gone: without it they never reach a script at all.
    """
    for signature in case["bip322_signatures"]:
        bip322.assert_as_valid(case["message"].encode(), case["address"], signature)
...
def test_error_vector(case: dict[str, str]) -> None:
    ...
    forgery (issue btclib-org/btclib#2181).
    ...
    octets, and answer one each (issue btclib-org/btclib#2197).
    """
...
def test_legacy_signature_is_accepted_for_p2pkh_alone() -> None:
    ...
    # all (issue btclib-org/btclib#1238)
...
def test_a_created_psbt_says_which_message_it_signs(
    ...
    """What the Creator writes, the Signer reads.

    The round trip of issue btclib-org/btclib#516.
