...
"""Tests for the `btclib_wallet.bip32` module."""
...
def _p2pkh_of(xkey: BIP32Key) -> str:
    ...
    builder takes a public key and no extended key (issue
    btclib-org/btclib#1188): what an xprv and its xpub both derive to is one
    public key, so both spellings answer one address. The network travels with
    it, the key's version bytes being where it is stated and the SEC octets
    carrying no network at all.
    """
...
def test_the_three_key_reads_answer_for_one_half_each() -> None:
    ...
    extended key (issue btclib-org/btclib#1188): the scalar of an xprv, the SEC
    octets of an xpub, the point of an xpub, and the SEC octets of either half.
    `network` and `compressed` are consistency checks throughout, an extended
    key stating both in its version bytes.
    """
...
def test_an_xkey_decodes_however_its_text_is_held() -> None:
    ...
    spelling decodes to what the text decodes to (issue btclib-org/btclib#1238).
    """
...
def test_bip32keydata_is_still_a_dataclass() -> None:
    ...
    three `Sig` classes (issue btclib-org/btclib#727): what `dataclasses`
    generates around a written-out `__init__` is what is left to check once the
...
def test_public_key_validation_does_not_lift(
    ...
    every level of every derivation path (issue btclib-org/btclib#615). What
...
def test_private_functions_do_not_validate(
    ...
    decoded, and on serializing the result (issue btclib-org/btclib#633). Two of
    the three are the rule -- input validation at the boundary, and the check
    the wrapper makes on what it hands back -- and the third was the private
...
def test_derive() -> None:
    ...
    # to it is `pub_keyinfo_from_xkey` of the derived one (issue
    # btclib-org/btclib#1188)
...
def test_the_tweaks_of_a_public_derivation_are_the_derivation(
    ...
    the signers as tweaks (BIP373, `btclib_wallet.psbt.musig2`) -- and what says
...
def test_cracking_refuses_a_child_its_own_assert_valid_refuses() -> None:
    """Both arguments through `_key_data_from_bip32_key`.

    Issue btclib-org/btclib#693.
...
def test_the_object_spellings_answer_what_the_text_ones_answer() -> None:
    """Each underscore spelling is the function above it.

    Issue btclib-org/btclib#886.
...
def test_the_object_spellings_validate_what_they_are_handed() -> None:
    """A public function validates its inputs, underscore or not.
...
def test_fingerprint_is_the_children_parent_fingerprint() -> None:
    ...
    here (issue btclib-org/btclib#1188).
    """
