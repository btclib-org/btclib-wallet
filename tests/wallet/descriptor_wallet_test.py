...
"""Tests for the `btclib_wallet.wallet.descriptor_wallet` module.
...
from btclib_wallet.wallet import BIP32KeyWallet, DescriptorWallet
...
# 36 days, the timelock the wallet of btclib-org/btclib#538 puts on its recovery
# path: the same design one output type further on
...
def test_the_psbt_carries_what_the_two_pre_images_cannot() -> None:
    ...
    # that and_v(v:older(n),X) emits, and not the OP_DROP the wallet of
    # btclib-org/btclib#538 writes by hand, which is why that one has no
    # descriptor at all
