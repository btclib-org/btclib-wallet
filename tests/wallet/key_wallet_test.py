...
"""Tests for the `btclib_wallet.wallet.key_wallet` module."""
...
def test_a_key_that_says_what_it_is_by_its_type() -> None:
    ...
    a wallet was handed is what the caller stated (issue
    btclib-org/btclib#1188).
    """
...
def test_add_refuses_an_extended_key() -> None:
    ...
    adds what comes back (issue btclib-org/btclib#1188).
    """
    wallet = KeyWallet()
...
def test_add_refuses_a_key_of_another_network() -> None:
    ...
    on its own, so the disagreement is the answer (issue
    btclib-org/btclib#1188).
    """
...
def test_add_derives_the_public_key_once(
    ...
    would derive the octets from again (issue btclib-org/btclib#1188).
