...
"""Tests for the vocabulary every `btclib_wallet.wallet` class answers to.
...
from btclib_wallet.wallet import (
    BIP32KeyWallet,
    DescriptorWallet,
    KeyGroup,
    RangedWallet,
...
def test_position_of_takes_every_spelling_of_an_output(
    ...
    answered (issue btclib-org/btclib#1238). An address is a `str` here and not
    octets -- its own bytes are read as a script, which is what they are.
    """
    wallet = build()
