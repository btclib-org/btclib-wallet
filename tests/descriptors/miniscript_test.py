...
"""Tests for `btclib_wallet.descriptors.miniscript`.
...
The two spending conditions of issue btclib-org/btclib#187 are checked as their
own case: both are p2wsh multisig with a timelocked recovery branch, taken from
a production custody protocol, and the point of them is that one maps to
miniscript opcode for opcode while the other does not -- ``v:older(n)`` writes
``<n> CHECKSEQUENCEVERIFY VERIFY`` where the script written by hand had ``<n>
CHECKSEQUENCEVERIFY DROP``, so that script is not miniscript at all. Both
answers were confirmed against Bitcoin Core v31.1: `getdescriptorinfo` accepts
each expression and `deriveaddresses` gives the address of the script computed
here.
"""
...
from btclib_wallet.descriptors.miniscript import (
    P2WSH,
    TAPSCRIPT,
    Miniscript,
    SpendContext,
...
from btclib_wallet.psbt.psbt import Psbt, finalize, taproot_sig_hash
from btclib_wallet.psbt.psbt_in import PsbtIn
from btclib_wallet.psbt.psbt_size import estimated_input_sizes
...
# the two production spending conditions of issue btclib-org/btclib#187, each as
# the miniscript it was claimed to be and as the script it was written as
TIMELOCKED_PRIMARY = (
...
def test_the_authorization_gate_maps_opcode_for_opcode() -> None:
    """Compile the second custody script, exactly as written.

    The script is issue btclib-org/btclib#187's.
...
def test_a_checksequenceverify_drop_is_not_miniscript() -> None:
    """Refuse the first custody script, which no fragment writes.
...
# authorization gate of issue btclib-org/btclib#187, which is the one with three
# branches
SOLVER_EXPRESSIONS = [
...
# inside tr() and btclib-org/btclib#503 could only read as a wsh()
...
# issue btclib-org/btclib#547's own shape: which branch a spend takes is decided
# before anything is estimated, and `max_witness_stack` cannot use that
...
def test_the_satisfaction_sizer_answers_for_the_branch_these_keys_build() -> None:
    ...
    # the bytes issue btclib-org/btclib#547 measures: paid on every recovery
    # spend by a sizer that does not know which branch it is, and not on this
    # one
