...
"""`btclib_wallet.hwi` against a real HWI, and whatever it is plugged into.
...
Bring your own, which is what the emulator item of issue btclib-org/btclib#381
comes down to: HWI's own suite starts a simulator per device family, each with
its own binary, image and udev rules, and vendoring that is the "large security
and maintenance surface unrelated to btclib's Bitcoin primitives" the issue
rules out. So the switches are two:
...
Two emulators are what CI brings, which is issues btclib-org/btclib#529 and
btclib-org/btclib#738 and not a change of that rule. `integration-hwi.yml` runs
a pinned Trezor Model T seeded with the mnemonic HWI's own suite uses, and a
Ledger under Speculos with the Bitcoin app built from a pinned tag -- twice, the
coin being compiled into a Ledger app, so the mainnet build answers the two
read-only tests and the testnet one signs. Both jobs set `BTCLIB_HWI_SIGN`,
because in neither is there anybody to press anything: HWI opens a udp Trezor
with `TrezorClientDebugLink`, which answers the button request itself, and
Speculos is given an automation file matching the text the app draws.
...
this and outside `btclib_wallet.hwi` altogether.
"""
...
from btclib_wallet.core_import import account_import_requests
from btclib_wallet.descriptors import at_index
...
def _one_device(hwi_executable: list[str], network: str) -> HwiSigner:
    ...
    The refusal of two devices is `btclib_wallet.hwi`'s own and is tested
    against the stand-in; here a session with two plugged in is one that cannot
