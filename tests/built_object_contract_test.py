...
btclib's rule that every public function validates its inputs is driven
by hand in `bool_contract_test.py` over the functions that answer a
`bool`, and btclib's own `input_validation_test.py` drives it over the
functions whose every required parameter is a library input type. Neither
...
to, and issue btclib-org/btclib#856 is where that ceiling is written down.
...
The shapes a missing check takes here are the ones issue
btclib-org/btclib#856 names: a sequence walked before it is checked
(`KeyGroup`'s `keys` and `origins`, `satisfaction_sizer`'s), an argument
reaching a comparison before its type is asked (`KeyGroup`'s `threshold`,
`Psbt.assert_valid`'s three int fields), an argument read for a field it
has not got (`assert_signatures_only`, `estimated_input_sizes`, both
sizers), and silence: a `sizer` of no callable type consulted only for
the inputs `estimated_input_sizes` cannot answer for on its own.
...
key as far as the type goes. One xpub handed where the list was meant would
be as many keys as it has characters.
...
changing an answer. btclib's `check_validity_test.py` owns that
convention.
"""
...
from btclib_wallet.descriptors.descriptors import miniscript_sizer, satisfaction_sizer
from btclib_wallet.psbt.psbt import Psbt, assert_signatures_only
