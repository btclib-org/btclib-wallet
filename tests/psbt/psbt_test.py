...
"""Tests for the `btclib_wallet.psbt.psbt` module."""
...
from btclib_wallet.psbt.psbt_in import _V2_FIELDS as _V2_INPUT_FIELDS
...
def test_taproot_signature_carries_its_sig_hash_type() -> None:
    """A taproot signature is 64 bytes, or 65 with its hash type.

    Issue btclib-org/btclib#122.
...
def test_parse_reads_a_stream() -> None:
    ...
    what taking BinaryData rather than Octets answers (issue
    btclib-org/btclib#179).
    """
...
def test_parse_refuses_what_follows_a_psbt_in_octets() -> None:
    ...
    thing an unchecked read length would buy, which is what
    btclib-org/btclib#138 was. Bitcoin Core refuses it in DecodeRawPSBT, the
    entry point that takes a buffer, and not in the Unserialize that reads a
    stream.
    """
    tx_in = TxIn(OutPoint(bytes(range(32)), 1), b"", 0xFFFFFFFF)
    tx_out = TxOut(2500000, "a914f987c321394968be164053d352fc49763b2be55c87")
...
def test_a_psbt_may_have_no_inputs() -> None:
    ...
    and Tx.serialize on the way back out (issue btclib-org/btclib#170).
    """
...
# issue btclib-org/btclib#249: the finalizer against btclib's own script engine
...
def test_what_the_finalizer_builds_the_engine_accepts(kind: str) -> None:
    """Finalize each single-key shape and run the result.

    Issue btclib-org/btclib#249.
...
def test_a_single_key_input_is_spent_with_its_public_key(kind: str) -> None:
    """The key beside the signature, and the empty script_sig.
...
def test_a_native_p2wsh_input_gets_no_script_sig() -> None:
    """No redeem script is no push, not a push of nothing.
...
def test_a_single_key_input_takes_one_signature() -> None:
    """A second signature is not a second half (issue btclib-org/btclib#249).
    ...
    same sig_hash, and it has to be: issue btclib-org/btclib#173 put a signature
    check in the finalizer ahead of this one, so a signature merely refiled
    under another key is refused as invalid before the count is ever reached.
    Two valid signatures are what isolate the count, and they are also the case
    that matters -- two participants who each signed.
    """
...
# issue btclib-org/btclib#305: the empty push BIP147 asks for is the script's,
# not the signatures'
...
def test_a_multisig_input_is_spent_with_the_element_the_script_pops(
    ...
    """OP_CHECKMULTISIG pops one element more than it reads.

    Issue btclib-org/btclib#305.
...
def test_the_dummy_is_the_multisig_script_and_not_a_second_signature() -> None:
    """A p2pk input holding two signatures gets no empty push.
...
def test_an_input_that_says_no_script_is_left_with_the_count() -> None:
    """No script is no answer, and the count is the only evidence.
...
def test_a_multisig_input_is_spent_in_the_order_the_script_lists_its_keys(
    ...
    """The Finalizer orders the signatures, and the dict does not.

    Issue btclib-org/btclib#431.
...
def test_a_multisig_input_pushes_the_threshold_and_not_every_signature(
    ...
    """A 1-of-2 both participants signed spends with one.
...
def test_a_multisig_input_short_of_the_threshold_is_not_finalized() -> None:
    """One signature does not satisfy a 2-of-2 (issue btclib-org/btclib#431).
...
def test_combining_merges_every_field_a_map_holds() -> None:
    """The preimages, the taproot fields and the musig2 maps too.

    Issue btclib-org/btclib#432.
...
def test_bip371s_own_psbts_are_signable(test_vector: dict[str, str]) -> None:
    """Every valid BIP371 psbt passes the Signer's pre-flight.

    Issue btclib-org/btclib#435.
...
# issue btclib-org/btclib#439: sign() over a KeyManager
...
def test_sign_falls_back_to_the_origin_the_key_manager_recognizes() -> None:
    ...
    Decision #1 of issue btclib-org/btclib#439: both travel on every call, key
    first and origin as the fallback a watch-only-shaped manager needs -- one
    that knows a fingerprint and path rather than every child key derived from
    it.
    """
...
def test_sign_skips_a_key_the_key_manager_does_not_hold() -> None:
    ...
    Decision #3 of issue btclib-org/btclib#439: None is not an error, so the
    other cosigner's key is left for its own turn rather than raised on.
    """
...
def test_sign_writes_the_taproot_script_path_signature() -> None:
    """A single-key leaf, signed and then finalized.

    Issue btclib-org/btclib#560.
...
def test_sign_appends_the_sig_hash_type_to_a_script_path_signature() -> None:
    """The 65-byte shape of a script path signature.
...
def test_an_answer_that_changes_the_transaction_is_refused() -> None:
    """Whichever field of it, and a sequence included.

    Issue btclib-org/btclib#381.
...
def test_an_answer_that_adds_a_preimage_or_a_vendor_field_is_refused() -> None:
    """Neither is a signature, so neither may appear.
...
def test_a_declared_map_count_is_bounded_before_it_is_allocated_for() -> None:
    """A count no transaction could have is refused, not believed.

    Issue btclib-org/btclib#569.
...
def test_the_three_entries_that_took_a_psbt_unasked() -> None:
    """`combine`, `prevouts` and `new_signers`.

    Issue btclib-org/btclib#692, under btclib-org/btclib#684.
