...
"""Tests for the `btclib_wallet.psbt.psbt_view` module.
...
from btclib_wallet.psbt import Psbt, PsbtView, ecdsa_sig_hash, taproot_sig_hash
from btclib_wallet.psbt.psbt import prevouts
from btclib_wallet.psbt.psbt_view import _read_var_int
...
def test_octets_are_one_whole_psbt() -> None:
    ...
    too (issue btclib-org/btclib#1238).
    """
...
def test_the_transaction_wide_hashes_are_computed_once(
    ...
    exactly the psbt that must not hash them once per input (issue
    btclib-org/btclib#164 is the same arithmetic for the object model). Counted
    rather than timed, and the answers are asserted to be the object model's: a
    precomputation that changed one of them would be an optimization around the
    wrong hash.
    """
    ...
    monkeypatch.setattr("btclib_wallet.psbt.psbt_view.PrecomputedTxData", counted)
