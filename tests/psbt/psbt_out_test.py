...
"""Tests for the `btclib_wallet.psbt.psbt_out` module."""
...
from btclib_wallet.psbt import Psbt, PsbtOut
...
def test_a_leaf_that_cannot_execute_is_storable() -> None:
    ...
    Core's PSBT does not look at a leaf script either, and issue
    btclib-org/btclib#123 is where refusing bytes for what an interpreter would
    say about them was settled. What cannot execute is unspendable, and a signer
    wanting to know says so by running it.
    """
...
def test_taproot_hd_key_paths_round_trips_through_dict() -> None:
    """Check a 32-byte pub_key survives the dict round trip.

    Issue btclib-org/btclib#311.
...
def test_taproot_bip32_from_dict_check_validity() -> None:
    ...
    # (issue btclib-org/btclib#264) -- deferred to PsbtOut.assert_valid rather
    # than refused here
