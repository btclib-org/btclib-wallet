...
"""Tests for the `btclib_wallet.psbt.psbt_in` module."""
...
from btclib_wallet.psbt import Psbt, PsbtIn
from btclib_wallet.psbt.psbt_in import (
...
def test_valid_sig_hash_type_matches_sig_hash_types() -> None:
    ...
    Nothing of a Literal exists at run time (issue btclib-org/btclib#273), so
    what mypy cannot check against sig_hash.assert_valid_hash_type's own
    frozenset, the suite does.
    """
...
def test_default_arguments_are_not_shared() -> None:
    """Check each PsbtIn gets its own final_script_witness.

    Issue btclib-org/btclib#139.
    """
