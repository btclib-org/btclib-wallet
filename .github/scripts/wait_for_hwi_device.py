...
`enumerate_devices` is this package's own, which makes this the same
call the fixture will make a second later, with `HwiDevice.is_usable`
deciding what counts: a device that answered a fingerprint and no error.
A Speculos that is up but has no app loaded enumerates with an error and
...
from btclib_wallet.hwi import HwiDevice, enumerate_devices
