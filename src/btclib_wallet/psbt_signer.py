...
module's: this is the contract such an adapter implements (issue
btclib-org/btclib#381).
...
class PsbtSigner(Protocol):
    ...
    def master_fingerprint(self) -> bytes:
        ...
        pay for the shape (issue btclib-org/btclib#814).
...
class WalletPolicyAddressDisplay(Protocol):
    ...
    of `btclib_wallet.hwi`, "Wallet policies", says why it has none of its own.
    """
...
class SignerDecorator:
    ...
    **Wrapping does not hide what the signer offers.** `AddressDisplay`,
    ...
    wrapper delegating that way satisfies `isinstance` on 3.11 and stops
    satisfying it on 3.12 and after -- the same wrapper, the same signer,
    a different answer per interpreter.
...
def display_policy_address(
    ...
    class being importable both as `btclib_wallet.descriptors.KeyExpression`
    and as `btclib_wallet.descriptors.key_expression.KeyExpression`, and a bare
...
class SoftwareSigner:
    ...
    def sign_message(self, message: Octets, der_path: DerPath) -> str:
        ...
        # (issue btclib-org/btclib#1188): resolve it to a `PrvKeyData` before
        # handing it over.
