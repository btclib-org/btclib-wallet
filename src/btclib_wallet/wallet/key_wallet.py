...
Both are the one-key half of `btclib_wallet.wallet`: the address is a hash of a
...
class KeyWallet(Wallet):
    ...
    def add(
        ...
        one (issue btclib-org/btclib#1188). A caller holding a WIF calls
    ...
    def sign(self, address: String, msg: Octets) -> bms.Sig:
        ...
        the address to `btclib_wallet.bip322.sign`, which signs for all four
        ...
        if info.script_type == "p2tr":
            ...
            err_msg += " message signing for taproot is btclib_wallet.bip322"
            raise BTClibValueError(err_msg)
...
class BIP32KeyWallet(KeyWallet, RangedWallet):
    ...
    def _derived_xkey(self, branch: int, index: int) -> BIP32KeyData:
        ...
        them build it and read it back inside one line (issue
        btclib-org/btclib#886). Which call it is, is what the caller wants of
        the key: the address builders and `ScriptPubKey.p2wpkh` take a public
        key, so `_derived_pub_key` below is what feeds them, and
        `b58.wif_from_prv_key` wants the scalar an xprv resolves to (issue
