...
# the alias was not configuration, and rebinding btclib_wallet.bip32.bip32.ec
...
def _cached_base58_decode(address: String) -> bytes:
    ...
    a memory leak (issue btclib-org/btclib#287).
...
class BIP32KeyData:
    ...
    write after the check that never came (issue btclib-org/btclib#727).
    `_BIP32KeyData` below is where derivation still needs to mutate one field at
    a time -- a sibling struct now, and not a subclass, frozen and non-frozen
...
def rootxprv_from_seed_(
    ...
    one seed repeats and `_cached_base58_decode` answers (issue
    btclib-org/btclib#886). The text is what `rootxprv_from_seed` is for, and
    nothing here stops answering it.
    """
...
def fingerprint(xkey: BIP32Key) -> bytes:
    ...
    outside this module's formats has a fingerprint (issue
    btclib-org/btclib#1188). Every reader of the four octets is this module's or
    sits above it -- `derive` below writes a parent's into each child it makes,
    `crack_prv_key_var` reads one back to tell whether a parent and a child are
    that pair, and `psbt`'s origins carry one to name a master.
...
def prv_keyinfo_from_xprv(
    ...
    defines it is the one that reads it (issue btclib-org/btclib#1188).
    ...
    xprv_data = _key_data_from_bip32_key(xprv)
    ...
    if xprv_data.key[0] != 0:
        # the offending key is public here, but never echo a
        # serialized xkey: the prefix already says what is wrong
        err_msg = f"not a private key: prefix 0x{xprv_data.key[:1].hex()}"
        raise InvalidPrvKeyError(err_msg)

    if network is None:
        network = network_from_xkeyversion(xprv_data.version)

    allowed_versions = xprvversions_from_network(network)
    if xprv_data.version not in allowed_versions:
        # never echo the xprv, which is a private key:
        # the version is the mismatching, non-secret, part
        err_msg = f"not a {network} key: version 0x{xprv_data.version.hex()}"
        raise InvalidPrvKeyError(err_msg)

    q = int.from_bytes(xprv_data.key[1:], byteorder="big")
...
def _pub_keyinfo_from_xpub(
    xpub_data: BIP32KeyData, network: str | None
) -> tuple[bytes, str]:
    """`pub_keyinfo_from_xpub` on a key already decoded and validated.

    Split off because `pub_keyinfo_from_xkey` below asks for it: the key
    it passes is the one `_xpub_from_xprv` has just built, so decoding it
    again through the public spelling would validate a key this module
    made, and `_assert_valid_key`'s on-curve test is most of what
...
def pub_keyinfo_from_xkey(
    xkey: BIP32Key, network: str | None = None
) -> tuple[bytes, str]:
    """Return the SEC octets and the network of an extended key, either half.

    The public key of an xprv is the public key of the xpub it neuters
    to, which is `fingerprint` above reading the same material the same
    way: a pair that answered two different public keys would not be a
    pair. So an address, a script and a key origin all ask this rather
    than asking whether they were handed the private half.

    Here and not in a converter, for the reason `prv_keyinfo_from_xprv`
    above is here: an extended key is this module's format (issue
    btclib-org/btclib#1188). An xprv costs the scalar multiplication
    `_xpub_from_xprv` performs, and an xpub costs nothing beyond the decode.

    No `compressed`: a BIP32 key is always compressed, so the only value
    that argument could carry is the one the key already states.
    """
    xkey_data = _key_data_from_bip32_key(xkey)
    if xkey_data.is_private:
        xkey_data = _xpub_from_xprv(xkey_data)
    return _pub_keyinfo_from_xpub(xkey_data, network)
...
class _BIP32KeyData:
    ...
    public function can trust one it is handed (issue btclib-org/btclib#727),
    and `dataclasses` refuses a mutable dataclass inheriting from a frozen one.
    `__prv_key_derivation`, `__pub_key_derivation` and the two path-walkers
    below mutate `chain_code`, `key`, `prv_key_int` and `parent_fingerprint`
    field by field across up to 255 levels of a path, for the measured
    performance reasons their own comments give; `_derive` builds one, mutates
    it, and hands a real `BIP32KeyData` back built from its final fields -- one
    allocation for the whole path, not one per level. Never validates and never
    coerces: every field always comes from a `BIP32KeyData` already valid or
    from this same loop, so there is nothing here for `check_validity` to gate.
...
def derive_(
    ...
    issue btclib-org/btclib#886 is about: a caller deriving from a key it holds
    pays neither the decoding of the argument nor the encoding of the answer,
    where the xprv text it would have built is a string the next call decodes
    again.
...
def _assert_valid_branch(
    ...
    # instead of arriving through `int_from_integer` (issue
    # btclib-org/btclib#1403, CONTRIBUTING.md's "This repository in particular")
    ...
    # raising (issue btclib-org/btclib#1413). Checked once, here, since this
    # function runs first at every call site and on the same `max_index`
...
def _assert_valid_address_index(address_index: int, max_index: int) -> None:
    ...
    # where that check lives (issue btclib-org/btclib#1413) -- a second one here
    # could never be reached and the coverage floor is what catches that
...
def derive_from_account_range_(
    ...
    Which is the larger half of what issue btclib-org/btclib#918 asked about,
    and not the half it named. The parse of the account key is one per *path*
    and not one per level -- `_PubKeyTweakChain` holds the point across the
    levels of a walk, so the branch key is never parsed from octets -- so what a
    range saves there is six percent of an address, which is not worth an entry
    point. The level is worth one.
