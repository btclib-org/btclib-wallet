...
from btclib_wallet.psbt.psbt_size import SIG_SIZE, SolutionSizer, _assert_input_types
...
def _pub_key_data(sec: bytes, network: str) -> PubKeyData:
    ...
    `assert_valid` reads off it (issue btclib-org/btclib#1188).
    """
...
class Descriptor(ABC):
    ...
    def __post_init__(self) -> None:
        ...
        # issue btclib-org/btclib#216 kept for everything else, not a new
        # refusal. Without it, two spellings of one network build descriptors
        # that are neither equal nor hashed alike, `network` comparing and
        # hashing as the plain field it is
    ...
    def index_of(
        ...
        its own change (issue btclib-org/btclib#540).
...
class WpkhDescriptor(Descriptor):
    ...
    def _satisfy(
        ...
        # whose script_sig is there at all (issue btclib-org/btclib#249). A
        # sh(wpkh()) gets its one push from the sh() above
...
class MiniscriptDescriptor(Descriptor):
    ...
    `btclib_wallet.descriptors.miniscript`'s.
