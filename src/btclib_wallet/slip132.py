...
def address_from_xpub(xpub: BIP32Key) -> str:
    ...
    for version, function in zip(version_list, function_list, strict=True):
        if network := network_from_key_value(version, xpub.version):
            ...
            # one takes (issue btclib-org/btclib#1188), and the prefix check
            # above is what makes the field readable as a public key here.
            # check_validity=False: that check is the prefix, and the size is
            # `BIP32KeyData`'s own
