...
from btclib_wallet.psbt.psbt_size import SolutionSizer, estimated_input_sizes
...
def _lock_time(
    ...
    them past -- which is what `btclib_wallet.psbt.psbt_view` does, and what
...
def _tx_in(psbt_in: PsbtIn, *, zeroed_sequence: bool) -> TxIn:
    ...
    lists two psbts with no inputs as valid (issue btclib-org/btclib#170) -- and
    every element of it is validated where it is held.
...
def _tx_out(psbt_out: PsbtOut, *, for_identifier: bool) -> TxOut:
    ...
    `btclib_wallet.psbt.psbt_view` -- passes False.
...
def _unsigned_tx(psbt: Psbt, *, for_identifier: bool) -> Tx:
    ...
    caller reading the maps one at a time -- `btclib_wallet.psbt.psbt_view` --
...
def _parse_global_map(
    ...
    two zero-input psbts BIP174 lists as valid (issue btclib-org/btclib#170).
...
def _read_unsigned_tx(
    ...
    holding one map at a time asks instead: `btclib_wallet.psbt.psbt_view`
...
def _assert_taproot_signable(psbt_in: PsbtIn) -> None:
    ...
    either or both (issue btclib-org/btclib#435):
...
class Psbt:
    ...
    def assert_valid(self) -> None:
        ...
        # as valid with no inputs (issue btclib-org/btclib#170). deserialize_tx
        # checks it the same way on the way in. The lock time is computed by the
        # very act of building it, so a psbt whose inputs require both kinds is
        # refused here, by _lock_time
    ...
    def from_dict(
        ...
        return cls(
            ...
            # psbts BIP174 lists as valid (issue btclib-org/btclib#170)
            [
    ...
    def serialize(self, *, check_validity: bool = True) -> bytes:
        ...
        if self.version == PSBT_V0:
            ...
            # the two zero-input psbts BIP174 lists as valid (issue
            # btclib-org/btclib#170)
    ...
    def parse(
        ...
        # the first one is read (issue btclib-org/btclib#569). Checked here
        # rather than in deserialize_count, which reads both counts and so could
        # not say which one it was refusing
    ...
    def b64decode(
        ...
        neither text nor bytes reaches `base64.b64decode` untouched, and is left
...
def _bip147_dummy(psbt_in: PsbtIn) -> list[bytes]:
    ...
    error and gets a dummy on top of it (issue btclib-org/btclib#305).
...
def _pushed_sigs(psbt_in: PsbtIn) -> list[bytes]:
    ...
    where the dummy belongs (issue btclib-org/btclib#431).
...
def _finalized_input(psbt_in: PsbtIn) -> tuple[bytes, Witness]:
    ...
    input" (issue btclib-org/btclib#249).
    """
...
def _spent_outputs(inputs: Iterable[PsbtIn]) -> list[TxOut]:
    ...
    `btclib_wallet.psbt.psbt_view` -- streams them past, and the whole previous
...
def sign(psbt: Psbt, key_manager: KeyManager) -> tuple[Psbt, list[int]]:
    ...
    Signer is `btclib_wallet.psbt.musig2` for the aggregate case. Every other
