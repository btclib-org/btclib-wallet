...
`btclib_wallet.psbt.musig2` derives MuSig2's, so there is no field for them:
...
the output does not commit to. The fourth way `btclib_wallet.psbt.musig2` reads
...
from btclib_wallet.psbt.psbt_utils import LEAF_HASH_SIZE
...
def _by_participant(
    ...
    `btclib_wallet.psbt.musig2` filters its own two maps: one input may carry a
...
def add_threshold_info(
    ...
    module's own. `btclib_wallet.psbt.musig2.add_participant_pub_keys` computes
...
def threshold_info(
    ...
    Public where `btclib_wallet.psbt.musig2`'s own reader is private: a MuSig2
