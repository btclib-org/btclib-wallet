...
def deserialize_sized_int(
    ...
    version as signed too, and `btclib_wallet.psbt.psbt` reads it unsigned,
...
def decode_taproot_bip32(
    ...
    # and treat each as its own leaf hash (issue btclib-org/btclib#1405)
...
def decode_musig2_participant_pub_keys(
    ...
    # bytes and treat each as its own participant (issue btclib-org/btclib#1405)
...
def assert_valid_taproot_signatures(signatures: list[bytes], what: str) -> None:
    ...
    signature Bitcoin Core accepts out of a psbt (issue btclib-org/btclib#122).
