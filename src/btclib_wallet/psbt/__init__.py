...
signer with less memory than the psbt takes (issue btclib-org/btclib#647). It is
beside `Psbt` and not another way to spell it: it reads, where every role above
rewrites, and what it hands back is the psbt's maps one by one rather than the
psbt. Its module docstring states what it holds between calls and what a stream
that changes underneath it costs.
...
btclib_wallet.psbt.psbt_utils is not exported, and this is where that decision
is recorded: serialize_bytes, deserialize_map, deserialize_tx, the
encode/decode_dict_bytes_bytes pair, serialize_dict_bytes_bytes,
serialize_hd_key_paths and assert_valid_unknown are how one field of one map is
written and read. They are called by psbt_in, psbt_out and psbt itself and by
nothing outside this package, and there were more of them in __all__ than there
were names for the format -- so a caller reading btclib_wallet.psbt was offered
the plumbing of a file format ahead of the psbt. Each is importable from the
module that defines it, which is where the test suite takes the other half of
that module from already.
"""

from btclib_wallet.psbt import frost, musig2, silent_payments
...
from btclib_wallet.psbt.psbt_view import PsbtView
