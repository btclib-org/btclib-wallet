...
# The seven hash types btclib.script.sig_hash.SIG_HASH_TYPES allows: the
# low five bits' four values -- DEFAULT, ALL, NONE, SINGLE -- alone and
# each ORed with ANYONECANPAY. Not an IntFlag over the whole byte: SINGLE
# == ALL | NONE, both being 3, so a flag decomposition would make SINGLE
# an alias of ALL | NONE, a bit structure the protocol does not have
# (issue btclib-org/btclib#273). Not an IntEnum either: the wire is wider
# than these seven -- `legacy` and `segwit_v0` mask rather than validate,
# and a byte such as 0x05 must still hash, being a value a mined signature
# can carry -- so btclib's assert_valid_hash_type keeps `hash_type: int`
# and checks a plain int against all 256 possible bytes (btclib's
# sig_hash_taproot_test.py's test_valid_sighash_type), which a
# Literal-typed parameter could not be called with. Narrowed only where
# the wire is already closed: PsbtIn.sig_hash_type below, checked against
# this same set once truthy, and tests/psbt/psbt_in_test.py checks the two
# stay equal
...
def _assert_valid_partial_sigs(partial_sigs: Mapping[bytes, bytes]) -> None:
    ...
    for pub_key, sig in partial_sigs.items():
        ...
        try:
            ...
            # sequence (issue btclib-org/btclib#129). Neither that byte nor the
            # key is checked against the signature here: both questions need the
            # transaction -- which sighash types the input admits, and which
            # hash the signature actually commits to -- so both are the
            # Finalizer's, and psbt.finalize asks them. Do not let a line of
